 ...
 * 包裹中的条目
 */
class HeXieItem:public UIItem{
public:
	/**
	 * 创建条目 展示的是道具图标 点击后根据类型做不同的处理 包裹额定大小 6 * 6 每个条目 64 * 64
	 */
	static HeXieItem*create(CCTexture2D * texture,const CCRect &rect);
 ...
 **/
class HeXieBag:public UISuperBag{
public:
	static HeXieBag* create(const CCSize &size,const char *pngName);
