...
void Stream::writeToFile(const char *fileName)
{
    ...
    if (file)
    {
		...
		fwrite(&size, sizeof(int), 1, file); // 原始报文大小
#ifdef  _USE_ZLIB_
		std::vector<char> buffer;
		buffer.resize(size * 1.5);
		this->compress(&buffer[0],size);
		
		size = content.size();
		fwrite(&size, sizeof(int), 1, file); // 压缩报文大小
#endif
...
void Stream::readFromFile(const char *fileName)
{
	...
	unsigned char * binary = cocos2d::CCFileUtils::sharedFileUtils()->getFileData(fileName,"rb",&nSize);
	if (!nSize)return;
	if (*binary == 's')
	{
		int size = 0;
		size = *(int*)(binary + 1); // 原始报文大小

		int fsize = size;
		int offset = 0;
#ifdef  _USE_ZLIB_
		fsize = *(int*) (binary + sizeof(int) + 1);
		content.resize(size);
		offset = 1+sizeof(int)*2;
#else
		offset = 1+sizeof(int);
#endif
		memcpy(&content[0],binary + offset,fsize);

#ifdef  _USE_ZLIB_
		char *buffer = new char[size];
		this->uncompress(buffer,size);
        delete[] buffer;
#endif	
	}
	/*
...
void Stream::addStore(Store & store)
{
	...
	memcpy(&content[offset],&store.coreData,sizeof(CoreData));
	memcpy((&content[sizeof(CoreData) + offset]),&(store.content[0]), store.coreData.size);
...
bool Stream::pickStore(Store &store)
{
	...
	{
		memcpy(&store.coreData,&content[offset] ,sizeof(CoreData));
		...
		if (sizeof(CoreData)+offset + store.coreData.size <= size())
			memcpy(&store.content[0],&content[sizeof(CoreData)+offset] ,store.coreData.size);
		else return false;
		offset += sizeof(CoreData) + store.coreData.size;
		return true;
	}
